...
connected: dict[tuple[int, int], bool] = {
    (i, j): False for i in range(len(boxes)) for j in range(i + 1, len(boxes))
}
while sum(connected.values()) < 1000:
...
sizes = sorted([len(cluster) for cluster in clusters.values()], key=lambda x: -x)
